...
#include <signal.h>
    bool cpu_loop_js() {
        BX_CPU(0)->cpu_loop();
        return (BX_CPU(0)->activity_state != BX_CPU_C::BX_ACTIVITY_STATE_ACTIVE);
    }
    extern void init_event_dispatcher_js();
    extern void set_timer_js();
}
...
int bx_init_main(int argc, char *argv[])
{
  ...
  SIM->get_param_enum(BXPN_BOCHS_START)->set(BX_QUICK_START);
//  SIM->get_param_bool(BXPN_RESTORE_FLAG)->set(1);
//  SIM->get_param_string(BXPN_RESTORE_PATH)->set("/state/");
