...
void BX_CPU_C::cpu_loop(void)
{
  ...
  while (1) {
    ...
    if (BX_CPU_THIS_PTR async_event) {
      if (handleAsyncEvent()) {
          handleAsyncEventCount++;
      }
      if (handleAsyncEventCount > 1) {
        // If request to return to caller ASAP.
        BX_CPU_THIS_PTR icount_last_relinquish_control = BX_CPU_THIS_PTR icount;
        return;
      }
    }

    if (BX_CPU_THIS_PTR icount >= BX_CPU_THIS_PTR icount_last_relinquish_control + MAX_INSTR_BEFORE_RELINQUISH_CONTROL) {
        BX_CPU_THIS_PTR icount_last_relinquish_control = BX_CPU_THIS_PTR icount;
        return;
    }
    ...
    for(;;) {
      ...
      if (BX_CPU_THIS_PTR icount >= BX_CPU_THIS_PTR icount_last_relinquish_control + MAX_INSTR_BEFORE_RELINQUISH_CONTROL) break;
